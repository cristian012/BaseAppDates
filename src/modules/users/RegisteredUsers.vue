    ...
    .global-content
        ...
        v-content
            v-container(fluid grid-list-lg)
                h1 {{$t('Usuarios inscritos')}}

                v-layout(row wrap v-if="registeredUsers.length")
                    v-flex.d-flex.pos-r.overflow-hidden(v-for="(user, i) in registeredUsers" :key="i" :class="$store.state.viewList == 0 ? 'flex-300' : 'flex-full'")
                        ...
                        .v-sheet(v-touch="{ left: () => swipe('Left', i), right: () => swipe('Right', i) }" :class="[swipeDirection == 'Right'+i ? 'move-right' : '', $store.state.viewList == 1 ? 'v-sheet-row' : '']")
                            ...
                            .v-sheet-content
                                ...
                                .set.mb-1
                                    ...
                                    .v-sheet-icon
                                        v-icon(size="16") $vuetify.icons.id
                                        h4 {{ user.id_type | placeholder('---') }} {{ user.id | placeholder('---') }}
                                .set.mb-1
                                    label {{$t('Ubicación')}}
                                    .v-sheet-icon
                                        v-icon(size="16") $vuetify.icons.place
                                        h4 {{ user.address | placeholder('---') }} {{ user.city | placeholder('---') }} {{ user.country | placeholder('---') }}
                                .set.mb-1
                                    label {{$t('Teléfono')}}
                                    .v-sheet-icon
                                ...
                                .set
                                    label {{$t('Fecha de inscripción')}}
                                    .v-sheet-icon
                                        v-icon(size="16") $vuetify.icons.date
                                        h4 {{ user.registration_date | formatDate('MMMM DD, YYYY') | capitalize | placeholder('---') }}
                ...
                not-found(v-else message="No se encontraron usuarios inscritos")

        footer-component(:drawer="$store.state.drawer" :mini="$store.state.mini")

        count-results(:current-counter="registeredUsers.length" :total-counter="registeredUsers.length" :filter="$store.state.filter")
